...
export class Road{
    ...
    constructor(x,width,laneWidth=34){
        this.x=x;
        this.laneCount= Math.floor((width-50) / laneWidth);
    ...
    //i punti ai quattro angoli della strada sono oggetti .
        const topLeft={x:x-width/2,y:this.top};
        const topRight={x:x+width/2,y:this.top};
        const bottomLeft={x:x-width/2,y:this.bottom};
        const bottomRight={x:x+width/2,y:this.bottom};
    ...
    draw(ctx){
        ...
        for(let i=0;i<=this.laneCount;i++){
            ...
            ctx.setLineDash((i==this.laneCount || i==0)?[]:[20,20]);
        ...
        this.borders.forEach(border=>{
            ctx.beginPath();
            ctx.strokeStyle = '#774513'; // Marrone scuro che potrebbe rappresentare il colore del legno
